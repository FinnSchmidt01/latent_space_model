...
state_dict = torch.load('toymodels/layer_indv_linear_mice_param_out12final.pth', map_location=device)
...
correlations = {"behavior1": torch.zeros((12,5), device=device), "behavior2": torch.zeros((12,5),device=device)}
total_correlations = {"behavior1": torch.zeros(12, device=device), "behavior2": torch.zeros(12,device=device)}
count_batches = 0
...
for batch_no, (data_key, data) in tqdm(
    ...
    mouse_correlations = {"behavior1": torch.zeros(12,device=device), "behavior2": torch.zeros(12,device=device)}
    for i in range(num_latents):  # Assuming 12 latent dimensions
        for j in range(2):  # Two behaviors
            ...
            correlation = torch.corrcoef(torch.cat((behavior_data.flatten(), latent_data.flatten())).view(2, -1))[0, 1]
...
for i in range(9,12):  # 12 latents
    plt.plot(x_labels, correlations["behavior1"][i].cpu().detach().numpy(), label=latent_labels[i])  # Plot each latent
