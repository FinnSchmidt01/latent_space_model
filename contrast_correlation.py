...
def run_linear_regression_analysis(
    ...
    for data_key, dct in aggregator.items():
        ...
        if feature_matrix:
            feature_matrix_mouse = feature_matrix[data_key] #shape [k,N]
            u, s, vh = torch.svd(feature_matrix_mouse) #u_q has shape k
            #express latents in the singualr dimesnion system. Thus, transform z into coordinates of singualr dimensions given
            #by u
            latents_cat = torch.matmul(latents_cat, u.cpu()).detach().numpy()
        
        else:
            latents_cat = latents_cat.numpy()
...
if __name__ == "__main__":
    ...
    model_path = "models/32_core_channels_latent_mice6_10best.pth"
    print(model_path)
    ...
    for data_key in data_keys:
        feature_q[data_key] = model.latent_feature[data_key + "_q"]
        feature_theta[data_key] = model.latent_feature[data_key + "_theta"]
