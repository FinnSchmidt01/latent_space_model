        ...
        if folder.startswith("dynamic"):
            ...
            if os.path.isdir(folder_path):
                ...
                if os.path.exists(mean_file_path):
                    ...
                    mean_tensor = torch.tensor(mean_array, dtype=torch.float32).to(device)
                
                    mean_variance_dict[folder + "_mean"] = mean_tensor
                
                if os.path.exists(variance_file_path):
                    variance_array = np.load(variance_file_path)
                    variance_tensor = torch.tensor(variance_array, dtype=torch.float32).to(device)
    ...
    for folder in folders:
        ...
        if folder.startswith("dynamic"):
            ...
            if os.path.isdir(folder_path):
                ...
                filtered_array = np.where(concatenated_array < 0.005, np.nan, concatenated_array)
                filtered_array = np.minimum(filtered_array,80) #cut outliers off
                first_row_valid_values = filtered_array[1, ~np.isnan(filtered_array[0, :])]
                k_params = []
                
                for i in range(filtered_array.shape[0]):
                    neuron_valid_values = filtered_array[i, ~np.isnan(filtered_array[i, :])]
                ...
                k, loc, theta = params
                x = np.linspace(min(first_row_valid_values), max(first_row_valid_values), 1000)
                ...
                plt.hist(first_row_valid_values, bins=50, density=True, edgecolor='black', alpha=0.6, label='Histogram of Valid Values')
