...
def full_objective(model, dataloader, data_key, *args, **kwargs):
        ...
        if loss_function == "ZIGLoss": 
            ...
            model_output = model(args[0].to(device), data_key=data_key,out_predicts = False, **kwargs)
...
model = ZIGEncoder(core = factorised_3d_model.core, 
...
model.load_state_dict(torch.load('toymodels2/nobehavior_pretrain_felxlayer_12pbest.pth', map_location=device))
...
def compute_correlations(model_output, original_data):
    correlations = np.array([
        scipy.stats.pearsonr(model_output[batch_ind, :, n].detach().cpu().numpy(), original_data[batch_ind, :, n].detach().cpu().numpy())[0]
...
def compute_correlations_latents(latents, behaviors):
    ...
    correlations = torch.zeros((n_latents, 2), dtype=torch.float32, device=latents.device)  # Two behaviors: pupil, speed
    
    for i in range(n_latents):
        for j in range(2):
            behavior_data = behaviors[:, :, j].flatten()
            latent_data = latents[:, :, i].flatten()
            correlation = torch.corrcoef(torch.stack((behavior_data, latent_data)))[0, 1]
            correlations[i, j] = correlation

    return correlations

# Function to plot and save the data for selected neurons
def plot_and_save_data_for_neurons(model_output, original_data,selected_neurons, paths, batch_index=7,theta=None,q=None,latent_theta=None,latent_q=None,latent = None):
    time_frames = np.arange(model_output.shape[1])
    for idx, neuron_idx in enumerate(selected_neurons):
        model_data = 10* model_output[batch_index, :, neuron_idx].detach().cpu().numpy()
        #model_data = (model_data - model_data.mean()) / model_data.std()
        original_data_slice = original_data[batch_index, :, neuron_idx].detach().cpu().numpy()
        ...
        plt.title(f'Neuron {neuron_idx} responses - Model vs. Original\nCorrelation: {correlation:.2f}')
        ...
        plt.close()
        if q is not None: #if model predictions for q and theta are given as well as their latent_features of the neuron
            ...
            if latent is not None:
                ...
                latent_impact_theta = torch.einsum('ij,j->i', latents[batch_index, :, :], latent_theta[:, neuron_idx]).cpu().detach().numpy()  # Latent impact for theta
                latent_impact_q = torch.einsum('ij,j->i', latents[batch_index, :, :], latent_q[:, neuron_idx]).cpu().detach().numpy()  # Latent impact for q
            #theta plotting
            plt.figure(figsize=(12, 6))
            plt.plot(time_frames, model_data, label='Model Output', color='blue')
            plt.plot(time_frames, theta_slice, label='Theta', color='red')
            if latent is not None:
                plt.plot(time_frames, latent_impact_theta[-len(time_frames):], label='Latent Impact on Theta', color='green', linestyle='--')
            ...
            if latent is not None:
                plt.plot(time_frames, latent_impact_q[-len(time_frames):], label='Latent Impact on q', color='green', linestyle='--')
...
def select_latents_for_plotting(correlations):
    ...
    max_indices = sorted_indices[:4, :]  # First two rows are the highest and second highest
    #min_indices = sorted_indices[-2:, :]  # Last two rows are the lowest and second lowest

    return max_indices


def plot_latents_with_behavior(latents, behaviors, selected_indices, behavior_names, batch_index=7,cca = False,savedir="plots/latent_behavior_correlation/"):
    """
    ...
    for i in range(2):  # Iterate over each behavior (pupil, speed)
        ax = axes[i]
        behaviors[batch_index, :, i] = behaviors[batch_index, :, i] - behaviors[batch_index, :, i].mean()
        behavior_data = 10*behaviors[batch_index, :, i].cpu().numpy()
        ax.plot(time_frames, behavior_data, label=f'{behavior_names[i]} Behavior', color='k', linewidth=2)
        if cca:
            print("..")
        else:
            for j in range(3):  # Plot the four selected latents (highest, second highest, lowest, second lowest)
                latent_idx = selected_indices[j, i]
                latent_data = latents[batch_index, :, latent_idx].detach().cpu().numpy()
                label_descriptions = ['Max Correlation', '2nd Max Correlation', '3rd Max Correlation']
                ax.plot(time_frames, latent_data, label=f'Latent {latent_idx+1} - {label_descriptions[j]}')
...
for batch_no, (data_key, data) in tqdm(
    ...
    model_params = model(batch_args[0].to(device), data_key=data_key,out_predicts = False, **batch_kwargs)
    ...
    if hasattr(model, 'encoder'):
        ...
        latents = model.encoder(batch_kwargs["responses"],data_key, model.mice[data_key][0:80,:])
    ...
    selected_neurons = [best_idx, second_best_idx, sorted_indices[2], sorted_indices[3]] + list(random_idxs)
    
    # Plot and save the figures
    plot_and_save_data_for_neurons(model_output, original_data, selected_neurons, paths,batch_index = 0)
    ...
    total_activity = original_data[batch_ind].sum(axis=0).detach().cpu().numpy()  # Sum over time, assuming batch_index = 0
    ...
    activity_labels = ["plots/latent_nobeh/highest_activity", "plots/nobeh_latent/2nd_highest_activity","plots/nobeh_latent/3rd_highest_activity","plots/nobeh_latent/4th_highest_activity"]
    activity_indices = [highest_activity_idx, second_highest_activity_idx,sorted_indices_by_activity[2],sorted_indices_by_activity[3]]
    ...
    plot_and_save_data_for_neurons(model_output, original_data, activity_indices, activity_labels,batch_index = batch_ind)#,theta=theta,q=q)


    #Plot latents against behavoir
    total_time = batch_kwargs["responses"].shape[2] #total number of time points before core 
    latents = model.encoder(batch_kwargs["responses"],data_key, model.mice[data_key][0:total_time,:])
    ...
    plot_latents_with_behavior(latents, behavior, max_indices, ['Pupil', 'Speed'],batch_index = batch_ind)
