...
def plot_response_histogram(responses, transformed_responses, bins=100):
    ...
    transformed_responses_flat = transformed_responses.cpu().detach().numpy().reshape(-1)
    ...
    counts, bin_edges, _ = ax[0].hist(responses_flat, bins=bins, alpha=0.7, label='Original Responses', density=True)
    counts = np.clip(counts, 0, max_frequency)  # Clip frequencies to max_frequency
    ax[0].cla()  # Clear the axes
    ax[0].bar(bin_edges[:-1], counts, width=np.diff(bin_edges), alpha=0.7, label='Original Responses', align='edge')
    ...
    counts, bin_edges, _ = ax[1].hist(transformed_responses_flat, bins=bins, alpha=0.7, color='g', label='Transformed Responses', density=True)
    counts = np.clip(counts, 0, max_frequency)  # Clip frequencies to max_frequency
    ax[1].cla()  # Clear the axes
    ax[1].bar(bin_edges[:-1], counts, width=np.diff(bin_edges), alpha=0.7, color='g', label='Transformed Responses', align='edge')
...
def plot_response_distriburion(model_path,outchannels,zig,latent,prior,encoder_dict,decoder_dict,cut = False,dropout = True, future = False,
                non_linearity = False,flow = False):
    """
...
dropout_prob: float in (0.25,1), if given masks given percentage of neurons. Can be at least 0.25, since first quarter of neurons has to be masked for evaluation on it
    ...
    if zig:
        ...
        factorised_3d_model.load_state_dict(torch.load(model_path, map_location=device),strict=False)
    ...
    for batch_no, (data_key, data) in tqdm(
        ...
        transformed_responses, _ = model.flow[data_key](original_data.permute(0,2,1), zero_mask.permute(0,2,1))
...
if __name__ == "__main__":
    ...
    plot_response_distriburion(model_path,out_channels,zig,latent,prior,encoder_dict=encoder_dict, decoder_dict = decoder_dict,
                cut = False, flow = True)