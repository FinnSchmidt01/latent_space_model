...
def model_predictions(
    model, dataloader, data_key, device="cpu", skip=50, deeplake_ds=False, prior = False,recursive=False, repeats = 10, n_samples =100,
     flow = False, dropout_prob = None, cell_coordinates = None
):
    """
    ...
    for batch in dataloader:
        ...
        with torch.no_grad():
            ...
            with device_state(model, device):
                ...
                if prior:
                    out = (
                    model.forward_prior(images.to(device), data_key=data_key,out_predicts = out_predicts,repeats=repeats,n_samples = n_samples,positions = positions, **batch_kwargs)
                )
                else: 
                    if (dropout_prob and (not neuron_mask)): #neuron_mask should be same for all batches
                        n_neurons = responses.shape[1] 
                        n_neurons_masked = int(n_neurons * (dropout_prob-0.25)) + 1 # first quarter is always masked
                        neuron_mask = random.sample(range(n_neurons // 4, n_neurons), n_neurons_masked)

                    out = (
                    model(images.to(device), data_key=data_key,neuron_mask = neuron_mask,out_predicts = out_predicts, positions = positions, **batch_kwargs)
                ...
                if model.flow:
                    ...
                    if model.flow_base == "Gaussian":
                        ...
                        psi_diag = model.psi[data_key].to(theta.device)  # Shape: (Neurons,)

                        # Apply reparameterization trick for sampling from Gaussian
                        # Sample epsilon from a standard normal distribution
                        epsilon = torch.randn((model.samples, *theta.shape), device=theta.device)  # Shape: (Samples, Batch, Time, Neurons)
                        epsilon = epsilon.permute(1, 2, 3, 0)  # Shape: (Batch, Time, Neurons, Samples)

                        # Reparameterize to obtain Gaussian samples with mean theta and variance psi_diag
                        gaussian_samples = theta.unsqueeze(-1) + epsilon * psi_diag.sqrt().unsqueeze(0).unsqueeze(0).unsqueeze(-1)  # Shape: (Batch, Time, Neurons, Samples)

                        # Apply inverese flow function
                        response_predictions = model.flow[data_key].invert_flow(gaussian_samples)
                    else:
                        gamma_samples = torch.distributions.Gamma(concentration=k, rate=q).rsample((model.samples,))  # Shape: (Samples, Batch, Time, Neurons)
                        gamma_samples = gamma_samples.permute(1, 2, 3, 0) + loc.unsqueeze(-1)  # Shape: (Batch, Time, Neurons, Samples)

                        # Apply inverese flow function
                        response_predictions = model.flow[data_key].invert_flow(gamma_samples)
